using UnityEngine;
using UnityEngine.Networking;
using System.Collections;
using System.Collections.Generic;
using TMPro;

public class WeatherRequest : MonoBehaviour
{
	private const string Url = "api.openweathermap.org/data/2.5/weather?q=London,uk&APPID=2908e48d30f25a47349ced6652e6f859";

	public Text​Mesh​Pro​UGUI textObject;
	public string initialText;
	public string LocationString;

	protected void Start()
	{
		initialText = textObject.text;
	}

	public void GetWeatherButton()
	{

		StartCoroutine(GetWeather());
	}

	private IEnumerator GetWeather()
	{
		UnityWebRequest request = UnityWebRequest.Get(Url);

		yield return request.SendWebRequest();

		if (request.result == UnityWebRequest.Result.Success)
		{
			string responseJson = request.downloadHandler.text;
			// Parse the JSON response
			WeatherData weatherData = JsonUtility.FromJson<WeatherData>(responseJson);

			Debug.Log(responseJson);
			//Debug.Log($"Temperature: {weatherData.current.Main.Temp}°C");
			
			textObject.text = initialText + responseJson.ToString();

			
		}
		else
		{
			Debug.LogError($"Request failed. Error: {request.error}");
		}
	}
}

[System.Serializable]
public class WeatherData
{
	public Root current;
}

[System.Serializable]
public class Coord
{
	public double Lon { get; set; }
	public double Lat { get; set; }
}
[System.Serializable]
public class Weather
{
	public int Id { get; set; }
	public string Main { get; set; }
	public string Description { get; set; }
	public string Icon { get; set; }
}

[System.Serializable]
public class Main
{
	public double Temp { get; set; }
	public double FeelsLike { get; set; }
	public double TempMin { get; set; }
	public double TempMax { get; set; }
	public int Pressure { get; set; }
	public int Humidity { get; set; }
}

[System.Serializable]
public class Wind
{
	public double Speed { get; set; }
	public int Deg { get; set; }
}


[System.Serializable]
public class Clouds
{
	public int All { get; set; }
}

public class Sys
{
	public int Type { get; set; }
	public int Id { get; set; }
	public string Country { get; set; }
	public int Sunrise { get; set; }
	public int Sunset { get; set; }
}


[System.Serializable]
public class Root
{
	public Coord Coord { get; set; }
	public List<Weather> Weather { get; set; }
	public string Base { get; set; }
	public Main Main { get; set; }
	public int Visibility { get; set; }
	public Wind Wind { get; set; }
	public Clouds Clouds { get; set; }
	public int Dt { get; set; }
	public Sys Sys { get; set; }
	public int Timezone { get; set; }
	public int Id { get; set; }
	public string Name { get; set; }
	public int Cod { get; set; }
}





































